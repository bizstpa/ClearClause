...
How it works: click the toolbar icon and the popup opens. If the page looks like a privacy policy (judged from its URL and title), the popup says so — and the toolbar icon itself shows a hint *before* you click, so likely policy pages stand out. You click **Scan this page**; the extension extracts the main text with Mozilla's [Readability](https://github.com/mozilla/readability) (run on a clone of the live page, dropping nav/footer/cookie chrome), runs the detectors, and shows the same readout as the web app. If a page yields little usable text (collapsed sections, multi-page policies, PDFs), the popup falls back to **scanning your current selection** or a **paste box** — never a dead end.

The toolbar-icon hint is computed by Chrome itself: a `declarativeContent` rule matches the URL against the same privacy signals *inside the browser*, so the extension only learns that a rule matched and never receives your URLs or browsing history. It needs no `tabs` permission and no host permissions — the only permission added for it is `declarativeContent`.

The extraction layer cleans the recovered text before the engine sees it, so on-page wayfinding doesn't get mistaken for policy language: in-page navigation and table-of-contents jump-link lists are stripped from the cloned page, standalone headings (`h1`–`h6` and `role="heading"`) are dropped rather than analyzed as sentences, and exact-duplicate blocks are collapsed. Two refinements catch styled subheaders that lack heading markup: a short line that recurs 3+ times collapses to a single instance, and a short, punctuation-less block that recurs is treated as a heading and dropped. A sentence severed across block edges (when a clause spills into a nested or inline block) is stitched back together: a line that doesn't end in clause-ending punctuation is rejoined to the next line when that line opens lowercase — a continuation signal — so a match displays as a whole sentence rather than a truncated fragment; capitalized lines (new headings or sentences) stay split, keeping the rejoin from creating run-ons. The cleaning errs toward keeping body text — when a block is ambiguous between navigation and content, it's kept. The detector engine itself is unchanged; only the quality of the text fed into it improved.
...
- **Reads a page only when you click.** Permissions are limited to `activeTab` + `scripting` + `declarativeContent`; there are no host permissions, no `<all_urls>`, no `tabs` permission, and no passive/background page reading. A minimal background service worker exists only to register the URL-pattern rule for the toolbar hint — it never reads page content or makes a network call. The page is read solely on your explicit action.
